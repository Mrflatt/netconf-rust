...
Resolution order for user: `--username` / `NETCONF_USERNAME`, then `User` from `~/.ssh/config`. Password is `--password` / `NETCONF_PASSWORD`. SSH `password` is tried first; if that fails and the server still offers `keyboard-interactive`, the same secret is used there (typical on Arista). `IdentitiesOnly yes` skips the password and uses only `IdentityFile` (ssh-agent if that key is loaded, otherwise the file). With `IdentitiesOnly yes` and no `IdentityFile`, OpenSSH default identity files under `~/.ssh/` are used. Without `IdentitiesOnly`, a password wins over `IdentityFile`; if neither is set, the CLI walks identities from `ssh-agent`. Remaining agent identities are tried after `IdentityFile` unless `IdentitiesOnly yes`.

`HostName`, `Port`, `User`, `IdentityFile`, `IdentitiesOnly`, `Compression`, `TCPKeepAlive` + `ServerAliveInterval`, algorithm prefs (`Ciphers`, `KexAlgorithms`, `MACs`, `HostKeyAlgorithms`), `UserKnownHostsFile`, and `ProxyJump` (any number of hops) are read from `~/.ssh/config`. Each jump authenticates with its own `IdentityFile` / `IdentitiesOnly` or the agent — the device password is not reused. Host keys default to `accept-new`: unknown hosts are pinned into `UserKnownHostsFile` or `~/.ssh/known_hosts`, a later key change is rejected. `--strict-host-key-checking yes` rejects unknown hosts too. `--strict-host-key-checking no` accepts any key.
